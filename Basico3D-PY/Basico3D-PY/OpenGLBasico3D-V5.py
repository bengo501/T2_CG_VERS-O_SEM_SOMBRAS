...
# Classe ObjetoTri atualizada para usar read_tri_file e desenhar triângulos
class ObjetoTri:
    def __init__(self, file_path):
        self.triangles = read_tri_file(file_path)  # Carrega triângulos do arquivo .tri

    def desenhar(self):
        glBegin(GL_TRIANGLES)
        for triangle in self.triangles:
            # Desenha cada triângulo com seus 3 vértices (9 valores)
...
def desenhar_objeto_tri(triangles, color):
    glColor3f(*color)  # Define a cor do objeto
    glBegin(GL_TRIANGLES)
...
def init():
    global Texturas
    glClearColor(0.6, 0.8, 1.0, 1.0)  # Valores RGB para um azul claro (0.6, 0.8, 1.0)    
    ...
    # Posiciona amigos e inimigos
    posicionar_amigos_e_inimigos()  # Posiciona os amigos e inimigos
